...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', (e) => {
            ...
            const conf = getStoreConfig();
            const cln = conf ? conf.phone.replace(/\D/g, '') : '88015483159';
            const waMsg = `Hello Haji Wadud Parts!\n\nName: ${name}\nEmail: ${email}${phone ? '\nPhone: ' + phone : ''}\n\nMessage: ${message}`;
            const waUrl = `https://wa.me/${cln}?text=${encodeURIComponent(waMsg)}`;
    ...
    /* ── 15. GLOBAL CONFIG HYDRATION  🆕 NEW ── */
    function applyConfig() {
        const config = getStoreConfig();
        if (!config) return;

        // Clean phone for WhatsApp links (keep digits only)
        const cleanPhone = config.phone.replace(/\D/g, '');

        // Phone numbers
        document.querySelectorAll('[data-config="phone-text"]').forEach(el => el.textContent = config.phone);
        document.querySelectorAll('[data-config="phone-link"]').forEach(el => el.href = `tel:${config.phone.replace(/\s+/g, '')}`);
        
        // Emails
        document.querySelectorAll('[data-config="email-text"]').forEach(el => el.textContent = config.email);
        document.querySelectorAll('[data-config="email-link"]').forEach(el => el.href = `mailto:${config.email}`);
        
        // Addresses
        document.querySelectorAll('[data-config="address-text"]').forEach(el => el.textContent = config.address);

        // WhatsApp links with dynamic messages
        document.querySelectorAll('[data-config="wa-link"]').forEach(el => {
            const defaultMsg = el.getAttribute('data-wa-text') || 'Hello Haji Wadud Parts!';
            el.href = `https://wa.me/${cleanPhone}?text=${encodeURIComponent(defaultMsg)}`;
        });

        // Floating WhatsApp button override
        const waFloat = document.querySelector('.whatsapp-float');
        if (waFloat) {
            waFloat.href = `https://wa.me/${cleanPhone}?text=Hello%20Haji%20Wadud%20Parts!`;
        }
    }

    applyConfig();
    window.applyConfig = applyConfig; // Expose globally if needed
});
