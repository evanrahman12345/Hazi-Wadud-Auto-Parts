...
const SHEET_CSV_URL = storeConfig.csvUrl;
...
function render() {
    ...
    grid.innerHTML = slice.map((p, idx) => {
        ...
        const confPhone = storeConfig ? storeConfig.phone.replace(/\D/g, '') : '88015483159';
        ...
        >
            ...
            <a
                href="https://wa.me/${confPhone}?text=${waText}"
                class="card-wa-btn"
                target="_blank"
...
function openModal(idx) {
    ...
    const confPhone = storeConfig ? storeConfig.phone.replace(/\D/g, '') : '88015483159';
    $('modalWaBtn').href = `https://wa.me/${confPhone}?text=${waText}`;
